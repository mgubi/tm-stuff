...
(define (dir-document doc)
  (with preamble `(document  
        (assign "font-family" "tt") (assign "font-base-size" "8pt") 
        (assign  "par-par-sep" "0.2fn") (assign "font" "Noto"))
    `(document
       (TeXmacs ,(texmacs-version))
       (style (tuple "generic"))
       (body (document (hide-preamble ,preamble) ,doc)))))

(define (dir-line dirname entry)
  (let* ((dir?        (url-directory? entry))
         (url         (url->string (if dir? (url-append "tmfs://local-dir/" (url-unroot entry)) 
                                            entry)))
         (short-name  (string-append (url->string (url-delta dirname entry)) 
                                     (if dir? "..." "")))
...
(define (dir-page name entries)
  (dir-document `(document (section* ,(string-append "File list " (url->string name)))
                           (with "par-columns" "2" 
                              (document  ,@(map (cut dir-line name <>) entries))))))
